...
alter table contes enable row level security;

-- Politiques temporaires "ouvertes" (à restreindre plus tard avec l'authentification Staff)
create policy "Public read" on contes
  for select using (true);

create policy "Public insert" on contes
  for insert with check (true);

create policy "Public delete" on contes
